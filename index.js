...
var { updateCMDStore, isbtnID, getCMDStore, getCmdForCmdId, connectdb, input, get, updb, updfb } = require("./lib/mongodb")
...
const sessdata = config.SESSION_ID.split("𝚀𝚄𝙴𝙴𝙽_𝙰𝙷𝙸𝙽𝚂𝙰=")[1];
...
console.log("Connecting ARROW-MD 🧬...");
...
let up = `*ARROW-MD connected successful ✅`;

await conn.sendMessage(ownerNumber + "@s.whatsapp.net", { image: { url: `https://i.ibb.co/DLDD8Zy/73a9c4e2ea3c4cf1.jpg` }, caption: up })
    ...
    conn.sendButtonMessage = async (jid, buttons, quoted, opts = {}) => {
                ...
                contextInfo: {
                    ...
                    forwardedNewsletterMessageInfo: {
                      ...
                      newsletterName: "🧚‍♂️⃝𝚀𝚄𝙴𝙴𝙽_𝙰𝙷𝙸𝙽𝚂𝙰-𝙼𝙳 𝚅3💕",
                      serverMessageId: 1
                    },
                    externalAdReply: { 
    title: '乡ARROW-MD',
    body: 'ＰＯＷＥＲＤ ＢＹ A-H-I-N-S-A ＭＤ',
    mediaType: 1,
    sourceUrl: "https://wa.me/94771098429",
    thumbnailUrl: "https://i.ibb.co/DLDD8Zy/73a9c4e2ea3c4cf1.jpg",
...
const isCreator = ["94771098429,94775713391"]
      .map((v) => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net")
      .includes(sender)    
...
await conn.sendMessage(from, { text: "❌ ```Another Bot's message Detected :``` 💥 *Removed By ARROW MD* ❗\nAnti Bot System on..." })
if ( config.ANTI_BOT == "true" && isBotAdmins ) {
await conn.sendMessage(from, { delete: mek.key })
await conn.groupParticipantsUpdate(from,[sender], 'remove')
  }}
    if ( mek.id.startsWith("QUEENAMDI") ) {
await conn.sendMessage(from, { text: "❌ ```Another Bot's message Detected :``` *💥 ARROW MD* ❗\n*Removed By ASITHA MD* ❗\nAnti Bot System on..." })
if ( config.ANTI_BOT == "true" && isBotAdmins ) {
await conn.sendMessage(from, { delete: mek.key })
await conn.groupParticipantsUpdate(from,[sender], 'remove')
  }}
